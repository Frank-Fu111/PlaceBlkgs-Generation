...
class ImageDataset_sample(Dataset):
    def __init__(self, data_file:str, seed:int = 42, is_int:bool = False):
        ...
        ## modified feature and encoding ##
        self.feature_paths: Dict[int, str] = {}
        self.encoding_paths: Dict[int, Dict[int, str]] = {}
    ...
    def update_data(self, data_file:str):
        ...
        for design_id, config_list in tqdm(self.config_list.items()):
            ...
            self.feature_paths[design_id] = feature_file

            self.encoding_paths[design_id] = {}
            for config_id in config_list:
                ...
                self.encoding_paths[design_id][config_id] = encode_file

    def get_run_ids(self):
        return list(self.run_config.keys())

    def get_feature_data(self, run_id:int) -> np.ndarray:
        path = self.feature_paths[run_id]
        # 使用 memory-mapped 避免 OS 一次性分配大量實體記憶體
        feat = np.load(path, mmap_mode='r')

        # 轉成一般 ndarray + float32
        feat = np.array(feat, dtype=np.float32)

        if feat.shape[0] == 19:
            feat[2, :, :] += feat[6, :, :]
            feat = np.delete(feat, 6, axis=0)
    ...
    def get_encode_data(self, run_id:int, encode_id:int) -> np.ndarray:
        path = self.encoding_paths[run_id][encode_id]
        enc = np.load(path, mmap_mode='r')
        # 如果你在產檔時有機會改成 uint8，這裡可以轉成 float32
    ...
    def get_data(self, run_id:int, encode1_id:int, encode2_id:int) -> \
        ...
        # class = 0：config1 better than config2
        # class = 1：config2 better than config1 
        # if drc1_count < drc2_count:
        #     cla = 0
        # else:
        #     cla = 1
...
def get_box_id(box:List[float], x_pitch:float = 2.0,
               y_pitch:float = 2.0) -> List[int]:
    llx_id = int(floor(box[0]/x_pitch))
    lly_id = int(floor(box[1]/y_pitch))
    urx_id = int(floor(box[2]/x_pitch))
    ury_id = int(floor(box[3]/y_pitch))
...
class SingleBestConfigDataset_complete(Dataset):
    ...
    def __getitem__(self, idx:int) -> torch.Tensor:
        encode1 = np.zeros((self.size_x, self.size_y, self.total_number_config), dtype=np.uint8)
        encode2 = np.zeros((self.size_x, self.size_y, self.total_number_config), dtype=np.uint8)
...
def get_encoding(boxes, config, size_x, size_y, total_number_config):
    encode = np.zeros((size_x, size_y, total_number_config), dtype=np.uint8)
