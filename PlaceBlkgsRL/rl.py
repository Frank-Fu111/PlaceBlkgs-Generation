...
def train_actor_critic_mc(envs, model, epochs=100, lr=0.001, gamma=0.9,
    ...
    mlflow.set_experiment(f"RL_1")
    
    with mlflow.start_run(run_name = suffix):
        mlflow.log_param("epoch", epochs)
        mlflow.log_param("learning_rate", lr)
        mlflow.log_param("gamma", gamma)
        mlflow.log_param("entropy_weight", entropy_weight)
...
if __name__ == "__main__":
    ...
    device = 0
    data_dir = "/home/frankfu/Tomography/util/step2_input/run_79"
    env1 = TomographyEnv(data_dir=data_dir, device=device, nsample = 6)
    data_dir = "/home/frankfu/Tomography/util/step2_input/run_11"
    env2 = TomographyEnv(data_dir=data_dir, device=device, nsample = 6)
    data_dir = "/home/frankfu/Tomography/util/step2_input/run_12"
    env3 = TomographyEnv(data_dir=data_dir, device=device, nsample = 6)
    # data_dir = "/home/frankfu/Tomography/util/step2_route_run_inputs/run_13"
    # env4 = TomographyEnv(data_dir=data_dir, device=device, nsample = 6)
    model = TomoLayer(num_channels=23, device = device)

    # model_path = "./best_model/rl_best_240421_075133.pth"
    # model.load_state_dict(torch.load(model_path))
    train_actor_critic_mc([env1, env2, env3], model, epochs=50, lr=0.001, gamma=0.99,
                          device=device, suffix = suffix, entropy_weight=0.01)
    
    # env.close()
    ## Save model
    print(f"Saving model to rl_{suffix}_des.pth")
    torch.save(model.state_dict(), f"./best_model/rl_{suffix}_des.pth")
